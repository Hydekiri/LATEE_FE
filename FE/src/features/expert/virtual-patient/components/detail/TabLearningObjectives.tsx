...
export function TabLearningObjectives({ patient, onSave, saving, readonly }: TabLearningObjectivesProps) {
    const [objectives, setObjectives] = useState<string[]>(
        patient.learningObjectives ? [...patient.learningObjectives] : [],
    );
        ...
        <div className="space-y-5">
            <div className="flex items-center justify-between">
                <h3 className="text-base font-black text-slate-800 flex items-center gap-2">
                    <Target className="w-5 h-5 text-[#235697]" />
                    Learning Objectives
                    ...
                    {objectives.map((obj, i) => (
                        <div key={i} className="flex items-start gap-3">
                            ...
                            <button
                                onClick={() => remove(i)}
                                disabled={saving}
                                className="mt-2 p-1.5 text-slate-300 hover:text-red-400 transition-colors disabled:opacity-40"
                            >

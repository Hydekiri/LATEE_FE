...
import { getClinicalCaseDetails } from "@/src/services/expert-clinicalcase-service";
...
const INITIAL_CASE_CONTEXT: CompleteClinicalCaseSchema = {
    ...
    createdByName: "Dr. Expert",
    eccId: "CRIT-001",
    createdAt: "2024-01-15T10:30:00Z",
    updatedAt: "2024-01-20T14:45:00Z",
    virtualPatientCount: 5,
    attemptCount: 1,
    avgScore: 85,
    labs: [
...
export default function ClinicalCaseDetailFeature({ caseId }: { caseId: string }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const data = await getClinicalCaseDetails(caseId);
                setCaseData(data);
            } catch (error) {
                console.error("Failed to fetch clinical case details:", error);
            }
        };
                ...
                <div className="bg-[#FFFFFF] border border-[#DDE7F0] rounded-xl p-4 flex items-center gap-3 shadow-xs">
                    ...
                    <div className="flex-1 min-w-0 relative group">
                        ...
                        <input
                            ...
                            className={getInputInteractiveClasses("caseType", caseData.caseType !== INITIAL_CASE_CONTEXT.caseType, "w-full bg-transparent border-b border-transparent hover:border-[#DDE7F0] text-[#173B67] font-mono font-bold text-xs outline-none mt-0.5 pr-6 transition-all rounded px-1")}
                        />
                        <PencilIcon className={`w-3 h-3 absolute right-0 bottom-1.5 pointer-events-none transition-colors duration-150 ${getPencilColorClass("caseType", caseData.caseType !== INITIAL_CASE_CONTEXT.caseType)}`} />
                    </div>
                </div>
                <div className="bg-[#FFFFFF] border border-[#DDE7F0] rounded-xl p-4 flex items-center gap-3 shadow-xs">
                    <div className="p-2.5 rounded-[10px] bg-[#235697]/10 text-[#235697]">
            ...
            <div className="flex flex-col">
                ...
                <div className="flex gap-1.5 text-xs font-semibold shrink-0 z-10 translate-y-px">
                    <button
                        ...
                        className={`px-5 py-3 rounded-t-xl border transition-all duration-150 ${activeTab === "history"
                    ...
                    <button
                        ...
                        className={`px-5 py-3 rounded-t-xl border transition-all duration-150 ${activeTab === "labs"
                            ? "bg-[#FFFFFF] text-[#235697] border-[#DDE7F0] border-b-transparent font-black shadow-2xs"
                            : "bg-[#1BA7D9] text-white/80 border-transparent hover:text-white hover:bg-[#1487AE]"
                            }`}
                    >
                    ...
                    <button
                        ...
                        className={`px-5 py-3 rounded-t-xl border transition-all duration-150 ${activeTab === "radiology"
                            ? "bg-[#FFFFFF] text-[#235697] border-[#DDE7F0] border-b-transparent font-black shadow-2xs"
                            : "bg-[#1BA7D9] text-white/80 border-transparent hover:text-white hover:bg-[#1487AE]"
                            }`}
                    >

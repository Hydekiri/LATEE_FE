...
// Import interface từ file types
import { EvaluationResultData } from '../types/submition'; 
...
export default function Results() {
    ...
    useEffect(() => {
        const fetchResult = async () => {
            ...
            setLoading(true);
            try {
                const response = await fetch(`http://localhost:5000/evaluation/api/evaluation/${resultId}/report`);
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center py-20 gap-4">
                <div className="w-12 h-12 border-4 border-[#235697] border-t-transparent rounded-full animate-spin"></div>
    ...
    if (!evalData || !evalData.epaScores) {
        return (
            <div className="bg-gray-50 border-2 border-dashed border-gray-200 rounded-3xl p-16 text-center text-gray-500 font-medium">
                Please complete the versioning process to see the results.
            </div>
        );
    }
                ...
                <div className="flex items-center w-full"> 
                    ...
                    <div className="flex flex-1 items-center gap-2">
                        {(() => {
                            ...
                            const totalAttemptsData = 1; // Có thể thay bằng evalData.attemptsCount nếu có
                ...
                <div className="col-span-7 flex flex-col justify-center">
                    ...
                    <ul className="space-y-1 text-md text-left">
                        ...
                        <li className="flex items-center gap-1">
                            ...
                            <span className="text-[#0E2A46] ml-1">{evalData.correctAnswer} / </span>
                            <span className="text-[#10B981] font-bold ml-1">{evalData.correctAnswer}</span>
                            <CheckCircleIcon className="w-4 h-4 text-[#10B981]" />
                        </li>

                        <li>
                            <span className="font-bold text-[#235697]">Case Type:</span> 
                            <span className="text-[#0E2A46] ml-1">{evalData.caseType}</span>
                        </li>
                        ...
                        <li>
                            ...
                            <span className="text-[#0E2A46] ml-1">{evalData.evaluation}</span>
                        </li>
                    </ul>
                </div>
            </div>

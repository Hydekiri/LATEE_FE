...
const generateRandomCourseId = (): string => {
        const randomNum = Math.floor(100 + Math.random() * 900);
        return `MED_${randomNum}`;
};
...
export default function AssessmentCreateForm({ onClose, onSuccess }: AssessmentCreateFormProps) {
    ...
    const [formData, setFormData] = useState<AssessmentFormData>({
        creatorId: "EXP_ANDREW",
        clinicalCaseId: "27553284",
        courseId: generateRandomCourseId(),
        moduleId: "ASS_EVALUATION_V1",
        title: "",
        specialty: "Neurology",
        topic: "",
        subtopic: "",
    ...
    const specialties: string[] = ["Cardiology", "Neurology", "Pediatrics", "Internal Medicine", "General Medicine"];
    ...
    const handleCreateAssessment = async (e: React.SyntheticEvent<HTMLFormElement>): Promise<void> => {
        ...
        try {
            ...
            if (!createRes.ok) {
                ...
                throw new Error(errorData.message || "Lỗi khi tạo Assessment");
            }
            ...
            if (newId) {
                ...
                if (!genRes.ok) throw new Error("Lỗi khi sinh câu hỏi");
    ...
    return (
        <div className="bg-white rounded-xl w-full max-w-2xl shadow-2xl flex flex-col max-h-[90vh] border border-slate-200 animate-in fade-in zoom-in duration-200">
            <div className="flex justify-between items-center px-6 py-4 border-b border-slate-100 bg-slate-50/50 rounded-t-xl">
                ...
                <button type="button" onClick={onClose} className="p-1.5 hover:bg-slate-200 rounded-md transition-colors">
            ...
            <form onSubmit={handleCreateAssessment} className="p-6 space-y-6 overflow-y-auto scrollbar-hide">
                ...
                <div className="grid grid-cols-5 gap-4">
                    ...
                    <div className="grid col-span-2">
                        ...
                        <div className="relative">
                            <input type="number" name="passingScorePercentage" className="form-input-full appearance-none pr-10" value={formData.passingScorePercentage} onChange={handleChange} />
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-1.5">
                    <label className="text-[13px] font-medium text-slate-700">Extra Note</label>
                    <textarea 
                        name="additionalPrompt"
                        className="form-input-full h-28 resize-none" 
                        placeholder="Provide more details for AI..." 
                ...
                <div className="flex gap-3 pt-4 sticky bottom-0 bg-white pb-2 border-t border-slate-50">
                    <button type="button" onClick={onClose} className="flex-1 px-4 py-2.5 rounded-lg border border-slate-200 text-sm font-medium text-slate-600 hover:bg-red-50 hover:text-red-600 transition-colors">Cancel</button>
                    <button disabled={isLoading} type="submit" className="flex-2 px-4 py-2.5 bg-[#235697] text-white rounded-lg text-sm font-medium hover:bg-[#1a4a82] transition-all shadow-sm flex items-center justify-center gap-2">
                        {isLoading ? <Loader2 className="animate-spin" size={16} /> : "Create & Generate"}
                    </button>
                </div>
            </form>

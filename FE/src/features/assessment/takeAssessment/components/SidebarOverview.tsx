...
export default function SidebarOverview({ 
    timeLeft, durationSeconds, totalQuestions, currentIdx, answers, onJumpToQuestion, questionIds, assessmentId
    ...
    return (
        <aside className="fixed top-[72px] right-0 w-80 h-[calc(100vh-72px)] bg-[#F6F6F6] border-l p-6 flex flex-col z-40">
                ...
                <div className="grid grid-cols-5 gap-3">
                    {Array.from({ length: totalQuestions }).map((_, i) => (
                        <button
                            ...
                            className={`h-11 rounded-xl font-bold border-2 transition-all ${
                                i === currentIdx ? "bg-[#1BA7D9] text-white border-[#1BA7D9]" : 
            ...
            <div className="flex flex-col gap-3 mt-auto pt-6">
                <button className="w-full py-3.5 bg-[#235697] text-white rounded-xl flex items-center justify-center gap-2">
            ...
            <ActionModal isOpen={showSubmitModal} onClose={() => setShowSubmitModal(false)}>
                <SubmitModalContent 

...
export default function Home_Header({ page }: NavbarProps) {
        ...
        <nav className="w-full top-0 left-0 bg-linear-to-r from-[#1ba7d9] to-[#235697] shadow-md z-50 flex justify-center sticky">
            <div className="w-[86%] flex items-center justify-between py-3">
                ...
                <div className="shrink-0">
                    <Link href="/home" aria-label="Go to Latee Homepage" className="relative block w-30 h-10 sm:w-37.5 sm:h-12 lg:w-45 lg:h-14">
                        <Image
                ...
                <div className="hidden xl:flex items-center">
                    ...
                    <div className="flex items-center gap-4">
                        {isMounted ? (
                            userInfo ? (
                                <>
                                    <button aria-label="Notifications" className="relative p-2 hover:bg-white/10 rounded-full transition">
                                    ...
                                    <div className="flex items-center gap-3 pl-2">
                                        <div className="w-10 h-10 rounded-full bg-white overflow-hidden relative border-2 border-white/50">
                                            <Image src="/images/ava1.jpg" alt="Avatar" fill sizes="40px" className="object-cover" />
                                        </div>
                            ...
                            <div className="flex items-center gap-4">
                                <div className="w-10 h-10 animate-pulse bg-white/20 rounded-full"></div>
                                <div className="w-32 h-10 animate-pulse bg-white/20 rounded-xl"></div>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <button aria-label="Open mobile menu" className="xl:hidden" onClick={() => setIsOpen(true)}>
            ...
            {isOpen && (
                <div className="fixed inset-y-0 right-0 z-60 flex justify-end w-full">
                    ...
                    <div className="w-72 bg-linear-to-b from-[#1ba7d9] to-[#235697] h-full shadow-2xl p-6 pt-10 relative animate-slide-left z-50 flex flex-col overflow-y-auto">
                        <button aria-label="Close mobile menu" onClick={() => setIsOpen(false)} className="absolute top-4 right-4 hover:bg-white/10 rounded-full p-1 transition">
                        ...
                        {isMounted && userInfo && (
                            <div className="flex items-center gap-3 mb-6 pb-6 border-b border-white/20">
                                <div className="w-12 h-12 rounded-full bg-white overflow-hidden relative border-2 border-white">
                                    <Image src="/images/LVP1.jpeg" alt="Avatar" fill sizes="48px" className="object-cover" />
                                </div>
                            ...
                            {isMounted && (
                                userInfo ? (
                                    ...
                                    <div className="flex flex-col gap-3">
                                        <Link href="/login" className="bg-white text-[#235697] rounded-xl text-center py-3 font-bold shadow-md hover:bg-gray-100 transition">
                                            Login
                                        </Link>
                                        <Link href="/signup" className="border-2 border-white text-white rounded-xl text-center py-3 font-bold hover:bg-white/10 transition">
                                            Sign up
                                        </Link>
                                    </div>
                                )
